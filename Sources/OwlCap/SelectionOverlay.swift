...
/// Hover a window, click it — the way macOS's own window capture works.
@MainActor
final class WindowPickerOverlay {
    private var window: OverlayWindow?
    private var view: WindowPickerView?

    var onPick: ((SCWindow) -> Void)?
    var onCancel: (() -> Void)?

    func show(windows: [SCWindow]) {
        let frame = ScreenGeometry.unionFrame
        let window = self.window ?? OverlayWindow(frame: frame, interactive: true)
        window.setFrame(frame, display: false)
        window.level = NSWindow.Level(rawValue: Int(CGWindowLevelForKey(.screenSaverWindow)) + 1)
        ...
        view.candidates = windows
        view.onPick = { [weak self] in self?.onPick?($0) }
        view.onCancel = { [weak self] in self?.onCancel?() }
...
private final class WindowPickerView: NSView {
    ...
    private func rect(for window: SCWindow) -> NSRect {
        ScreenGeometry.toAppKit(window.frame).offsetBy(dx: -origin.x, dy: -origin.y)
    }

    override func mouseMoved(with event: NSEvent) {
        let point = convert(event.locationInWindow, from: nil)
        let hit = candidates.first { rect(for: $0).contains(point) }
    ...
    override func mouseDown(with event: NSEvent) {
        if let highlighted { onPick?(highlighted) } else { onCancel?() }
    }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        guard let highlighted else {
            let text = "Move over a window to choose it   •   Esc to cancel"
            let attrs: [NSAttributedString.Key: Any] = [
                .font: NSFont.systemFont(ofSize: 15, weight: .medium),
                .foregroundColor: NSColor.white.withAlphaComponent(0.92),
            ]
        ...
        let target = rect(for: highlighted)
        NSColor.clear.setFill()
        target.fill(using: .copy)
        NSColor.controlAccentColor.setStroke()
        let border = NSBezierPath(rect: target)
        border.lineWidth = 3
        border.stroke()
    }
}
